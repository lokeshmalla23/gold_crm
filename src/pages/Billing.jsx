...
import NumberInput, { toNumber } from '../components/ui/NumberInput';
...
export default function Billing() {
  ...
  const [goldRateInput, setGoldRateInput] = useState(String(GOLD_RATE));
  ...
  const goldRate = toNumber(goldRateInput) || GOLD_RATE;
  ...
  const splitTotal = toNumber(splitAmt1) + toNumber(splitAmt2);
  ...
  const handleSplitAmt1 = (raw) => {
    setSplitAmt1(raw);
    const val = toNumber(raw);
    if (raw === '') {
      setSplitAmt2(total > 0 ? String(total) : '');
      return;
    }
    const capped = Math.min(val, total);
    if (val > total) {
      setSplitAmt1(String(total));
      setSplitAmt2('0');
    } else {
      setSplitAmt2(String(total - capped));
    }
  };
  const handleSplitAmt2 = (raw) => {
    setSplitAmt2(raw);
    const val = toNumber(raw);
    if (raw === '') {
      setSplitAmt1(total > 0 ? String(total) : '');
      return;
    }
    const capped = Math.min(val, total);
    if (val > total) {
      setSplitAmt2(String(total));
      setSplitAmt1('0');
    } else {
      setSplitAmt1(String(total - capped));
    }
  };
  ...
  const holdBill = () => {
    ...
    setCart([]); setCustomer(null); setDiscountInput('');
  ...
  const resumeHeld = (h) => {
    ...
    setCart(resumed);
    setDiscountInput(h.discount ? String(h.discount) : '');
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 h-[calc(100vh-8rem)]">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5 flex flex-col">
        ...
        <div className="mb-4">
          ...
          <NumberInput value={goldRateInput} onChange={setGoldRateInput} className="mt-1 w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:border-amber-400 focus:ring-2 focus:ring-amber-100 outline-none" />
        </div>
        ...
        <div className="space-y-2 text-sm border-t border-gray-100 pt-3">
          ...
          <div className="flex items-center justify-between">
            ...
            <NumberInput value={discountInput} onChange={setDiscountInput} className="w-24 px-2 py-1 border border-gray-200 rounded text-sm text-right" />
          </div>
          ...
          {paymentMode === 'Split' && (
            <div className="mt-3 p-3 bg-amber-50 border border-amber-200 rounded-lg space-y-2">
              ...
              <div className="flex gap-2">
                ...
                <NumberInput value={splitAmt1} onChange={handleSplitAmt1} placeholder="Amount" className="w-28 px-2 py-1.5 border border-gray-200 rounded text-sm text-right" />
              </div>
              <div className="flex gap-2">
                ...
                <NumberInput value={splitAmt2} onChange={handleSplitAmt2} placeholder="Amount" className="w-28 px-2 py-1.5 border border-gray-200 rounded text-sm text-right" />
              </div>
